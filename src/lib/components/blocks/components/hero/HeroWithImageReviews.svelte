<script lang="ts">
	...
	const hasImage = $derived(imageSrc.length > 0);
	const displayTitle = $derived(highlight ? `${title} ${highlight}` : title);
...
<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
	<div class="grid gap-4 md:grid-cols-2 md:items-center md:gap-8 xl:gap-20">
		<div>
			<h1
				class="block text-3xl font-bold text-foreground sm:text-4xl lg:text-6xl lg:leading-tight"
			>
				{#if highlight}
			...
			<div class="mt-7 grid w-full gap-3 sm:inline-flex">
				<Button href={ctaPrimaryHref} variant="default" class="gap-x-2">
					{ctaPrimaryText}
					<svelte:component this={ChevronRight} class="size-4 shrink-0" />
				</Button>
				<Button href={ctaSecondaryHref} variant="outline">
					{ctaSecondaryText}
				</Button>
			</div>
						...
						<div class="py-5">
							<div class="flex gap-x-1">
								{#each generateStarArray(review.rating) as starType}
									<svelte:component
										this={Star}
										class={cn('size-4', getStarClass(starType))}
									/>
								{/each}
							</div>
